...
.hi.orange[Expectation]

- Definition
- Properties


.hi.turquoise[Variance]
...
First we give the mathematical definition of .hi.orange[expectation] for discrete random variables.
...
Suppose $X$ has a .hi.red_pink[uniform distribution] over $(a,b)$ denoted 
...
Let $X$ be a random variable with $E(X)< \infty$. Then the .hi.turquoise[variance] of $X$ is:
...
While $E(X)$ is called the .hi.green[first moment], we call $E(X^2)$ the .hi.orange[second moment] of X. 
...
--
$$ 
E(X^2) = 0^2 \cdot (\frac{1}{4})+ 1^2 \cdot (\frac{1}{2}) + 2^2 \cdot  (\frac{1}{4}) = \frac{1}{2} + 1 = \frac{3}{2} ^{\color{red_pink}*}
$$

This means that 
$$V(X)=E(X^2)-[E(X)]^2 = \frac{3}{2}-1^2 = \frac{1}{2}$$

.footnote[.red_pink[<sup>*</sup>].hi[Never] square probabilities when solving \(E(X^2)\). Only square \(X\) values]
...
- $Var(aX+bY) = a^2Var(X)+b^2Var(Y)+2ab\cdot Cov(X,Y).$<sup>.red_pink[*]</sup> 

.footnote[.red_pink[<sup>*</sup>] We will discuss more about covariance later, when $X$ and $Y$ are independent then the covariance is zero]
