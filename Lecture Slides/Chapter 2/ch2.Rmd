...
for(i in 1:5) {
	p <- ggplot(data= sample_df %>% filter(sample == i), aes(x= values)) +
	...
	png_path <- here("Lecture Slides", "Chapter 2", glue::glue("frame{i}.png"))
...
sample_df <- sample_df %>% group_by(sample) %>% mutate(mean = mean(values))

png_paths <- c()

for(i in 1:5) {
	mean <- sample_df %>% filter(sample == i) %>% slice(1) %>% pull(mean)
...
# write_csv(stats, here("Lecture Slides", "Chapter 2", "bball_90_99.csv"))

stats <- read_csv(here("Lecture Slides", "Chapter 2", "bball_90_99.csv"))

stats <- stats %>% 
	mutate(Rodman = if_else(namePlayer == "Dennis Rodman", "Dennis Rodman", "Other")) %>%
	filter(slugSeason == "1991-92")
