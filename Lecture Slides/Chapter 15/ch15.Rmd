...
$${\color{ruby}\bar{X}} = \frac{1}{n} \sum_{i=1}^{n} X_i$$

$${\color{ruby}\displaystyle s^2} = \frac{1}{n-1} \sum_{i=1}^{n} (X_i - \bar{X})^2$$ 


Because $X_1,..., X_n$ are random variables, any sample statistic ${\color{ruby} \hat{\theta}} = g(X_1,...,X_n)$ is itself a random variable! 

That means, there is some distribution for the values of ${\color{ruby} \hat{\theta}}$
...
p <- ggplot(df) + 
	...
	kfbmisc::theme_kyle(base_size = 28) + 
	labs(x = NULL, y = "Sample Means")

ggsave(here("Lecture Slides", "Chapter 15", "sample_dist_diff_n.png"), p, width=8, height=6)
